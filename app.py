...
# Traitement des requêtes de suppression issues du clic droit
...
if "delete_node" in query_params:
    node_to_del = query_params["delete_node"]
    if node_to_del not in ["s", "t"] and node_to_del in st.session_state.nodes:
    ...
    st.query_params.pop("delete_node", None)
    st.rerun()

if "delete_edge" in query_params:
    try:
        edge_to_del = json.loads(query_params["delete_edge"])
        st.session_state.edges = [e for e in st.session_state.edges if not (e["source"] == edge_to_del["source"] and e["target"] == edge_to_del["target"])]
    except Exception:
        pass
    st.query_params.pop("delete_edge", None)
    st.rerun()
...
with st.sidebar:
    ...
    # 3. SUPPRESSION VIA LISTE
    st.subheader("🗑️ Supprimer via Menu")
...
# -----------------------------------------------------------------------------
# CANVAS INTERACTIF HTML5 AVEC MENU CONTEXTUEL CLIC DROIT
# -----------------------------------------------------------------------------
    ...
    <body>
        ...
        <script>
            ...
            function deleteNode(nodeName) {{
                const url = new URL(window.parent.location.href);
                url.searchParams.set('delete_node', nodeName);
                window.parent.history.replaceState({{}}, '', url.toString());
            ...
            function deleteEdge(src, dst) {{
                const url = new URL(window.parent.location.href);
                url.searchParams.set('delete_edge', JSON.stringify({{source: src, target: dst}}));
                window.parent.history.replaceState({{}}, '', url.toString());
                window.parent.location.reload();
            }}
            ...
            canvas.addEventListener('contextmenu', (e) => {{
                ...
                for (let n of nodes) {{
                    ...
                    if (Math.hypot(mouseX - sx, mouseY - sy) <= 20) {{
                        ...
                        if (n === 's' || n === 't') {{
                            contextMenu.innerHTML = `<div style="color: #94A3B8; cursor: not-allowed;">❌ Impossible de supprimer '${{n}}'</div>`;
                        }} else {{
                            contextMenu.innerHTML = `<div onclick="deleteNode('${{n}}')">🗑️ Supprimer le nœud '${{n}}'</div>`;
                        }}
                ...
                for (let e_item of edges) {{
                    if (positions[e_item.source] && positions[e_item.target]) {{
                        ...
                        if (dist <= 10) {{
                            contextMenu.style.left = `${{e.clientX - rect.left}}px`;
                            contextMenu.style.top = `${{e.clientY - rect.top}}px`;
                            contextMenu.innerHTML = `<div onclick="deleteEdge('${{e_item.source}}', '${{e_item.target}}')">🗑️ Supprimer l'arête '${{e_item.source}} ➔ ${{e_item.target}}'</div>`;
                            contextMenu.style.display = 'block';
                            return;
                        }}
                    }}
            ...
            // Masquer le menu au clic gauche
...
with col_main:
    st.subheader("🕸️ Représentation du Graphe (Faites un clic droit sur un nœud ou une arête pour le supprimer)")
...
with col_info:
    ...
    if st.session_state.edges:
        ...
        st.dataframe(edge_table, use_container_width=True)        updated_positions = json.loads(query_params["canvas_data"])
        for node, pos in updated_positions.items():
            if node in st.session_state.positions:
                st.session_state.positions[node] = pos
    except Exception:
